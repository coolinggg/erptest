...
function Dialog(content, options)
{
    ...
    this.resetPos = function()
    {
        ...
        if(options.center)
        {
            ...
            var top = ($(window).height() - dialog.height()) / 2-100;
    ...
    this.setContent = function(c)
    {
        ...
        if('object' == typeof(c))
        {
            ...
            case 'img':
                div.html('加载中...');
                $('<img alt="" />').load(function(){div.empty().append($(this));this.resetPos();})
                    .attr('src',c.value);
                break;
            case 'url':
                div.html('加载中...');
                $.ajax({url:c.value,
                        success:function(html){div.html(html);this.resetPos();},
    ...
    this.show = function()
    {
        if(undefined != options.beforeShow && !options.beforeShow(this))
        {   return;  }
